<template>
  <div class="registpage">
    ...
    <van-checkbox v-model="chkbtnAgree" class="agreeCheck" @change="monitorData">
      Đồng ý điều khoản
      <!-- <img slot="icon" slot-scope="props" :src="props.checked ? activeIcon : inactiveIcon" /> -->
      <i
        slot="icon"
        v-if="!chkbtnAgree"
        class="iconfont my-icon-check"
        style="display:block;font-size:18px;color:#999999"
      ></i>
      <i
        slot="icon"
        v-if="chkbtnAgree"
        class="iconfont my-icon-check1"
        style="display:block;font-size:18px;color:#79E582"
      ></i>
    </van-checkbox>
    <van-button
      :disabled="btnCommitDisable"
      @click="handleRegist"
      class="btnlogin"
      v-bind:class="{'btnCommitDisable': btnCommitDisable}"
      type="default"
    >Đăng ký</van-button>
    <van-dialog
  ...
  methods: {
    ...
    monitorData() {
      if (
        ...
        this.btnCommitDisable = false;
      } else {
        this.btnCommitDisable = true;
      }
    },
    //注册
    handleRegist() {
      ...
      loginServiceApi.regist(param).then(res => {
        if (res.code == 0) {
